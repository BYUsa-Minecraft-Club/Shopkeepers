...
public class FoxCustomizations {
    ...
    public static List<ShopkeeperCustomization<FoxEntity>> getFoxCustomizations(FoxEntity fox) {
        List<ShopkeeperCustomization<FoxEntity>> customizations = new ArrayList<>();
        customizations.add(new FoxVariantCustomization(fox.getVariant()));
        customizations.add(FoxPositionCustomization.forFox(fox));
        return customizations;
    }

    private record FoxVariantCustomization(FoxEntity.Variant variant) implements ShopkeeperCustomization<FoxEntity> {
        ...
        @Override
            ...
            public ShopkeeperCustomization<FoxEntity> setNext(FoxEntity shopkeeper) {
                FoxEntity.Variant next = CustomizationUtils.nextAlphabetically(shopkeeper.getVariant());
                ((FoxEntityVariationSetter) shopkeeper).invokeSetVariant(next);
                return new FoxVariantCustomization(next);
            }
        }

    private record FoxPositionCustomization(FoxPosition position) implements ShopkeeperCustomization<FoxEntity> {
        ...
        @Override
        public ShopkeeperCustomization<FoxEntity> setNext(FoxEntity shopkeeper) {
            FoxPosition next = CustomizationUtils.nextAlphabetically(FoxPositionCustomization.forFox(shopkeeper).position());
